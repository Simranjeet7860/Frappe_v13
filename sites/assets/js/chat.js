(function () {
  ...
  class ChatBubble {
    ...
    setup() {
      ...
      var bubble_visible = this.parent.is_desk === true ? 'd-none' : '';
    ...
    setup_events() {
      var me = this;
      $('#chat-bubble, .chat-cross-button').on('click', function () {
        me.change_bubble();
      });
    }
  }
  ...
  function get_date_from_now(dateObj, type) {
    var sameDay = type === 'space' ? '[Today]' : 'h:mm A';
    var elseDay = type === 'space' ? 'MMM D, YYYY' : 'DD/MM/YYYY';
    var result = moment(dateObj).calendar(null, {
      sameDay: sameDay,
      lastDay: '[Yesterday]',
      lastWeek: elseDay,
      sameElse: elseDay,
  ...
  function is_date_change(dateObj, prevObj) {
    var curDate = moment(dateObj).format('DD/MM/YYYY');
    var prevDate = moment(prevObj).format('DD/MM/YYYY');
  ...
  async function create_guest(ref) {
    var email = ref.email;
  ...
  function set_notification_count(type) {
    var current_count = frappe.Chat.settings.unread_count;
    if (type === 'increment') {
  ...
  class ChatSpace {
    ...
    make_messages_html(messages_list) {
      ...
      messages_list.forEach(function (element) {
        var date_line_html = this$1.make_date_line_html(element.creation);
        this$1.message_html += date_line_html;

        var message_type = 'sender';

        if (element.sender_email === this$1.profile.user_email) {
          message_type = 'recipient';
        } else if (this$1.profile.room_type === 'Guest') {
          if (this$1.profile.is_admin === true && element.sender !== 'Guest') {
            message_type = 'recipient';
          }
        }
        this$1.message_html += this$1.make_message(
          element.content,
    ...
    upload_file(file) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
              ...
              try {
                var error$1 = JSON.parse(xhr.responseText);
                var messages = JSON.parse(error$1._server_messages);
                var errorObj = JSON.parse(messages[0]);
        ...
        form_data.append('docname', this$1.profile.room);
    ...
    make_message(content, time, type, name) {
      ...
      var $recipient_element = $(document.createElement('div')).addClass(
        message_class
      );
      var $message_element = $(document.createElement('div')).addClass(
        'message-bubble'
      );
      ...
      var file_name = content.substring(n + 1) || '';
      var $sanitized_content;
  ...
  class ChatRoom {
    ...
    setup() {
      ...
      var last_message = this.sanitize_last_message(this.profile.last_message);
    ...
    sanitize_last_message(message) {
      var sanitize_last_message = $('<div>').text(message).html();
    ...
    set_last_message(message, date) {
      var sanitized_message = this.sanitize_last_message(message);
    ...
    setup_events() {
      ...
      this.$chat_room.on('click', function () {
        if (typeof this$1.chat_space !== 'undefined') {
          this$1.chat_space.render();
        } else {
          this$1.chat_space = new ChatSpace({
            $wrapper: this$1.$wrapper,
            chat_list: this$1.chat_list,
            profile: this$1.profile,
          });
        }
        if (this$1.profile.is_read === 0) {
          mark_message_read(this$1.profile.room);
          this$1.set_as_read();
        }
      });
    }
  ...
  class ChatAddRoom {
    constructor(opts) {
      this.user = opts.user;
      this.users_list = frappe.user.get_emails().concat( ['Administrator']);
        ...
        fields: [
          {
            ...
            default: 'Group',
            onchange: function () {
              var type = this$1.add_room_dialog.get_value('type');
              var is_group = type === 'Group';
              this$1.add_room_dialog.set_df_property('room_name', 'reqd', is_group);
              this$1.add_room_dialog.set_df_property('users', 'reqd', is_group);
              this$1.add_room_dialog.set_df_property('user', 'reqd', !is_group);
            },
            reqd: true,
          },
          {
        ...
        action: {
          primary: {
            ...
            onsubmit: function (values) {
              var users = this$1.add_room_dialog.fields_dict.users.get_values();
              var room_name = values.room_name;
              if (values.type === 'Direct') {
                users = [values.user];
                room_name = 'Direct Room';
              }
              this$1.handle_room_creation(room_name, users, values.type);
              this$1.add_room_dialog.hide();
            },
          },
        },
      });
        ...
        action: {
          primary: {
            ...
            onsubmit: function (values) {
              ...
              this$1.user_settings_dialog.hide();
            },
          },
        },
      });
  ...
  class ChatList {
    ...
    setup_rooms() {
      ...
      this.rooms.forEach(function (element) {
        ...
        this$1.chat_rooms.push([
          profile.room,
          new ChatRoom({
            $wrapper: this$1.$wrapper,
            $chat_rooms_container: this$1.$chat_rooms_container,
            chat_list: this$1,
            element: profile,
          }) ]);
      });
    ...
    fitler_rooms(query) {
      for (var i = 0, list = this.chat_rooms; i < list.length; i += 1) {
        var room = list[i];

        var txt = room[1].profile.room_name.toLowerCase();
    ...
    move_room_to_top(chat_room_item) {
      this.chat_rooms = [
        chat_room_item ].concat( this.chat_rooms.filter(function (item) { return item !== chat_room_item; }) );
    }
    ...
    setup_socketio() {
      ...
      frappe.realtime.on('latest_chat_updates', function (res) {
        ...
        var chat_room_item = me.chat_rooms.find(
          function (element) { return element[0] === res.room; }
        );
  ...
  class ChatForm {
    ...
    async validate_form() {
      try {
        ...
        var res = await create_guest(form_values);
  ...
  frappe.Chat = class {
    ...
    async setup_app() {
      try {
        var token = localStorage.getItem('guest_token') || '';
        var res = await get_settings(token);
